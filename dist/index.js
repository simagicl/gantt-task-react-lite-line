  ...
  return React__default.createElement("div", {
    ...
    return React__default.createElement("div", {
      ...
      onClick: function onClick() {
        var row = document.querySelector('.scroll');
        if (row) {
          var scrollTop = row.scrollTop;
          var scrollLeft = row.scrollLeft || 0;
          localStorage.setItem("ganttScrollTop", String(scrollTop));
          localStorage.setItem("ganttScrollLeft", String(scrollLeft));
          localStorage.setItem("ganttSelectedTaskId", t.id);
          localStorage.setItem("ganttScrollTimestamp", String(Date.now()));
          alert("Posici\xF3n guardada - Top: " + scrollTop + "px, Left: " + scrollLeft + "px, Tarea: " + t.id);
        }
      }
    }, React__default.createElement("div", {
...
var VerticalScroll = function VerticalScroll(_ref) {
  ...
  React.useEffect(function () {
    var saved = localStorage.getItem("ganttScrollTop");
    var savedTimestamp = localStorage.getItem("ganttScrollTimestamp");
    var currentTime = Date.now();
    if (scrollRef.current && saved && savedTimestamp) {
      var timestamp = parseInt(savedTimestamp);
      if (currentTime - timestamp < 30000) {
        scrollRef.current.scrollTop = parseInt(saved);
        console.log("Posici\xF3n restaurada: " + saved + "px");
      }
    }
  }, [ganttFullHeight]);
  React.useEffect(function () {
...
var GridBody = function GridBody(_ref) {
  var tasks = _ref.tasks,
    ...
    todayLineColor = _ref.todayLineColor,
    rtl = _ref.rtl;
  var y = 0;
  var gridRows = [];
  ...
  var tickX = 0;
  var ticks = [];
  var today = React__default.createElement("g", null);
  var todayLineX = 0;
  for (var i = 0; i < dates.length; i++) {
    ...
    if (i + 1 !== dates.length && date.getTime() < now.getTime() && dates[i + 1].getTime() >= now.getTime() || i !== 0 && i + 1 === dates.length && date.getTime() < now.getTime() && addToDate(date, date.getTime() - dates[i - 1].getTime(), "millisecond").getTime() >= now.getTime()) {
      ...
      today = React__default.createElement("g", null, React__default.createElement("rect", {
        x: tickX,
        y: 0,
        width: columnWidth,
        height: y,
...
var Calendar = function Calendar(_ref) {
  ...
  var getCalendarValuesForWeek = function getCalendarValuesForWeek() {
    ...
    for (var i = dates.length - 1; i >= 0; i--) {
      ...
      var bottomValue = "" + weekPrefix + getWeekNumberISO8601(date);
      bottomValues.push(React__default.createElement("text", {
        key: date.getTime(),
        y: headerHeight * 0.8,
        x: columnWidth * (i + +rtl),
  ...
  var getCalendarValuesForDay = function getCalendarValuesForDay() {
    ...
    for (var i = 0; i < dates.length; i++) {
      var date = dates[i];
      var bottomValue = " " + getLocalDayOfWeek(date, locale, "narrow") + date.getDate().toString();
      bottomValues.push(React__default.createElement("text", {
        key: date.getTime(),
        y: headerHeight * 0.8,
        x: columnWidth * i + columnWidth * 0.5,
...
var BarDisplay = function BarDisplay(_ref) {
  ...
  var isProgressComplete = progressWidth >= width;
  return React__default.createElement("g", {
...
var Project = function Project(_ref) {
  ...
  var projectWidth = task.x2 - task.x1;
  ...
  var isProgressComplete = task.progressWidth >= projectWidth;
  return React__default.createElement("g", {
    tabIndex: 0,
  ...
  }), task.name !== "TRAMITACIÓN" && React__default.createElement(React__default.Fragment, null, React__default.createElement("rect", {
    fill: barColor,
    x: task.x1,
    width: projectWidth,
    y: task.y,
...
var TaskItem = function TaskItem(props) {
  ...
  var isProgressComplete = task.progress === 100;
  var labelClassName = style.barLabel + " " + (!isTextInside ? style.barLabelOutside : '');
  return React__default.createElement("g", {
  ...
  }, taskItem, !isProgressComplete && React__default.createElement("text", {
    x: getX(),
...
var TaskGanttContent = function TaskGanttContent(_ref) {
  ...
  return React__default.createElement("g", {
    ...
    return task.barChildren.map(function (child) {
      var dynamicArrowIndent = task.project === "AGRUPADOR1" ? arrowIndent : arrowIndent / 2;
    ...
    var taskArrowIndent = task.project === "AGRUPADOR1" ? arrowIndent : arrowIndent / 2;
...
var Gantt = function Gantt(_ref) {
  var tasks = _ref.tasks,
    ...
    _ref$weekPrefix = _ref.weekPrefix,
    weekPrefix = _ref$weekPrefix === void 0 ? 'W' : _ref$weekPrefix,
    ...
    _ref$todayLineColor = _ref.todayLineColor,
    todayLineColor = _ref$todayLineColor === void 0 ? "rgba(252, 248, 227, 0.5)" : _ref$todayLineColor,
  ...
  var _useState0 = React.useState(0),
    scrollY = _useState0[0],
    setScrollY = _useState0[1];
  var _useState1 = React.useState(-1),
    scrollX = _useState1[0],
    setScrollX = _useState1[1];
